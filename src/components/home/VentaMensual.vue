<template>
  <v-container class="contenido">
    <v-row>
      <v-col>
        <h3 class="titulo">Venta mensual</h3>
      </v-col>
    </v-row>
    <v-row class="informacion">
      ...
      <v-col>
        ...
        <div class="detalle_valor">Venta del mes</div>
      </v-col>
      <v-col>
        <div class="valor">{{meta}}</div>
        <div class="detalle_valor">Meta de ventas del mes</div>
      </v-col>
    </v-row>
  </v-container>
</template>
  ...
  methods: {
    ...
    fetchVentas(){
      axios.get('http://localhost:8080/api/kpis.json')
        .then(resp=>{
          console.log(resp.data[0].venta_mensual);
